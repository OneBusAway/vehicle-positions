...
| `ACCESS_TOKEN_TTL` | `15m` | Optional | How long an API access token stays valid. Clients renew with `POST /api/v1/auth/refresh` before it elapses; the value is returned as `expires_in` on login and refresh. Unparseable: warn and default. **A non-positive value refuses to start** — it would issue tokens that are already expired. Does not affect the admin UI's browser session, which is fixed at 24 hours. |
