...
| `ACCESS_TOKEN_TTL` | `24h` | Optional | How long an API access token stays valid. Clients renew with `POST /api/v1/auth/refresh` before it elapses; the value is returned as `expires_in` on login and refresh. The default is unchanged from before refresh tokens existed because the Android driver app cannot refresh yet — lower it only once every client handles `401 → refresh → retry`. Unparseable: warn and default. **A non-positive value refuses to start** — it would issue tokens that are already expired. Does not affect the admin UI's browser session, which is fixed at 24 hours. |
| `REFRESH_TOKEN_TTL` | `168h` | Optional | How long a refresh token stays valid (7 days). Each refresh consumes the token presented and issues a replacement with a fresh window. Unparseable: warn and default. **A value not longer than `ACCESS_TOKEN_TTL` refuses to start** — a refresh token that expires first has nothing to renew. |
